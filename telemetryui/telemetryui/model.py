...
class Record(db.Model):
    ...
    def get_recordcnts_by_build():
        sql = text('SELECT build, count FROM processed.builds ORDER BY build::Numeric ASC')
        builds_count = db.engine.execute(sql).fetchall()
        return builds_count

    @staticmethod
    def get_builds():
        sql = text('SELECT build FROM processed.builds')
        blds = db.engine.execute(sql).fetchall()
        return sorted(blds, key=lambda x: LooseVersion(x[0]), reverse=True)
    ...
    def get_classifications(with_regex=False):
        sql = text('SELECT classification FROM processed.classifications')
        classifs = db.engine.execute(sql).fetchall()
        if with_regex:
            classes = [Record.expand_class(c[0].split('/')) for c in classifs]
    ...
    @staticmethod
    def get_os_map():
        sql = text('SELECT system_name, build FROM processed.os_map')
